...
import { IQueryItem, IQueryItemWithId, IShortItem, ItemId, Locale } from "@sotah-inc/core";
import { debounce } from "lodash";

import { getItems } from "../../api/data";
import { getItemIconUrl, getItemTextValue, qualityToColorClass } from "../../util";

const ItemSuggest<T> = Suggest.ofType<IQueryItem<T extends IQueryItemWithId>>();

export interface IOwnProps<T> {
  autoFocus?: boolean;
  idBlacklist?: number[];
  closeOnSelect?: boolean;
  itemIdActiveList?: number[];
  initialResults?: Array<IQueryItem<T>>;

  onSelect(item: T): void;
}

type Props<T> = Readonly<IOwnProps<T>>;

function inputValueRenderer<T>(result: IQueryItem<T>): string {
